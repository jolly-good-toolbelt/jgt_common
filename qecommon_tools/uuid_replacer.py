...
class UUIDLineReplacer(object):
    ...
    def uuid_mappings(self):
        '''Return a list of lines (suitable for ``.writelines``) of all the substitutions done.'''
        return ['# {} -> {}\n'.format(replacement, uuid)
                for uuid, replacement in sorted(self.uuid_map.items(), key=lambda item: item[1])]
...
def main():
    description = ('Utility for replacing UUIDs in files with easier to read placeholders. '
    ...
    parser.add_argument('--template', '-t', type=str, help='UUID replacement template',
