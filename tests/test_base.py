...
def test_filter_dict_keep_value(dict_to_filter, value_to_keep):
    new_dict = qecommon_tools.filter_dict(dict_to_filter, keep_value=lambda x: x == value_to_keep)
...
def uuids_with_extra_text():
    ...
    return [extra_text + target_uuid, extra_text + target_uuid + extra_text,
            target_uuid + extra_text]


def broken_uuids():
    target_uuid = str(uuid4())
    return [target_uuid[1:], target_uuid[:-1], target_uuid.replace('-', ''),
            target_uuid.replace('-', '+')]
...
def uuids_with_isolated_extra_text():
    target_uuid = str(uuid4())
    extra_text = qecommon_tools.generate_random_string()
    return [target_uuid, '/' + target_uuid, extra_text + '-' + target_uuid + '-',
            target_uuid + '/' + extra_text]
...
@pytest.mark.parametrize('uuid_with_isolated_extra_text', uuids_with_isolated_extra_text())
